...
Google Analytics (GA4, measurement ID `G-QFW1PYC388`) is wired in via a `gtag.js` snippet duplicated at the top of every page's `<head>` — same caveat as the fonts link: it has to be copied into any new page, and isn't recoverable by looking at a shared file if it's ever missing from one.
