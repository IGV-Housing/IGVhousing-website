...
Design tokens (CSS custom properties on `:root`, redeclared per page): `--evergreen` (dark teal-green, primary brand color), `--cream` (off-white), `--coral` (orange accent), `--ink`, `--line`. The site uses **Montserrat only** — a prior serif accent font (Bitter) was removed site-wide for visual uniformity, so don't reintroduce a second typeface without an explicit request.

Montserrat is **self-hosted**, not loaded from Google Fonts — the two variable-weight woff2 files live at `public/assets/fonts/montserrat-variable.woff2` (weights 300–800, normal) and `montserrat-italic-variable.woff2` (italic), with `@font-face` rules and a `<link rel="preload">` for the regular file duplicated in every page's `<head>` (same per-page duplication caveat as everything else in `<head>`). This replaced a Google Fonts `<link>` with `display=optional`, which was silently failing to render Montserrat at all in Chrome on any page load where the font wasn't already cached (the whole point of `optional` is to skip rendering rather than risk a late swap causing layout shift — but that meant real, user-visible failures, not just a rare edge case). Self-hosting removes the fetch time that made `optional` unreliable, so `font-display: swap` is safe here and used instead — it guarantees Montserrat eventually renders.
